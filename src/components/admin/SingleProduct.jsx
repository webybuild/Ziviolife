...
import EditTechnicalSpecificationsForm from "./EditTechnicalSpecificationForm";
import editFormState from "../../atoms/editFormStateAtom";
import loaderState from "../../atoms/loaderStateAtom";
import { useNavigate } from "react-router-dom";

function SingleProduct({ showButton }) {
  ...
  const [showEditForm, setShowEditForm] = useRecoilState(editFormState);
  ...
  async function fetchSingleProductInfo() {
    try {
      ...
      console.log(product)
      const res = await axios.get(url2 + "/product/" + product);
      console.log(res)
  ...
  async function deleteProduct() {
    try {
      setLoader(true);
      const res = await axios.delete(url2 + "/product/" + info.id);
      toast.success(res.data.message, { autoClose: 3000 });
      ...
      <div className="">
        ...
        {showEditForm && (
          <Modal
            ...
            Component={EditTechnicalSpecificationsForm}
            width={"full"}
          />
        )}
